...
trait StorageDocument
{
    ...
    public function downloadStoragePurchase($folder, $filename)
    {
        return Storage::disk('tenant')->download($folder . DIRECTORY_SEPARATOR . $filename);
    }

    public function downloadStorageAnnexed($folder, $filename)
    {
    ...
    public function downloadDigitalFile($root)
    {
        if (!Storage::disk('tenant')->exists($root)) {
            return null;
        } else {
            return Storage::disk('tenant')->download($root);
        }
    }
    ...
    private function setData($filename, $file_type, $root)
    {
        ...
        $this->_filename = ($extension) ? $filename . '.' . $extension : $filename;
