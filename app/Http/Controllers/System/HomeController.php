...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $avail = new Process(array('df', '-m', '-h', '--output=pcent', '/', '|', 'tail', '-n 1'));
        $avail->run();
        ...
        $i_used = '';
        if ($disc_used != 0) {
            $inodes = new Process(array('df', '-i', '/', '|', "awk '{print $5}'", '|', 'tail', '-n 1'));
            $inodes->run();
            $i_used = $inodes->getOutput();
        }
        $i_used = $i_used != "" ? substr($i_used, 0, -1) : 0;

        $df = new Process(array('du', '-sh ' . storage_path(), '|', 'cut -f1'));
        $df->run();
        $storage_size = $df->getOutput();
        $storage_size = $storage_size != "" ? substr($storage_size, 0, -1) : 0;

        $id = new Process(array('git', 'describe', '--tags'));
        $id->run();
