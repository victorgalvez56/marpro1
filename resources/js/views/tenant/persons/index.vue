<template>
    <div>
        ...
        <div class="card mb-0">
                        ...
                        <td class="text-right">
                            ...
                            <template v-if="typeUser === 'admin'">
                                <button
                                    ...
                                    v-if="!row.is_invited"
                                    @click.prevent="clickSendInvitation(row.id)"
                                >
                                    Invitar al portal
                                </button>
                                ...
                                <button
                                    ...
                                    @click.prevent="clickUsuarios(row.id, row.name)"
                                >
                                    Usuarios
                                </button>
                            </template>
            ...
            <users
                :showDialog.sync="showDialogUsuarios"
                :personId="recordId"
...
export default {
    ...
    methods: {
        ...
        clickUsuarios(recordId = null, personName="") {
            this.recordId = recordId;
            this.personName = personName;
            this.showDialogUsuarios = true;
        },

        clickSendInvitation(id) {
            this.$http
                .get(`/${this.resource}/send_invitation/${id}`)
                .then(response => {
                    if (response.data.success == true) {
                        this.$eventHub.$emit("reloadData");
                        this.$message.success(response.data.message); //Success
                    } else {
                        this.$message.error(response.data.message); //no se connect
                    }
                });
        }
    }
